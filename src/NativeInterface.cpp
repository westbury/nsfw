...
NativeInterface::NativeInterface(const std::string &path, std::shared_ptr<EventQueue> queue, bool followSymlinks) {
  mNativeInterface.reset(new NativeImplementation(queue, path, followSymlinks));
}
