    ...
    it('fires events for the symlink itself when followSymlinks is false', (done) => {
      ...
      nsfw(watchPath, storeEvents, { debounceMS: DEBOUNCE, followSymlinks: false })
        ...
        .then(() => {
          ...
          expect(firedEvents[0]).toEqual({ action: nsfw.actions.DELETED, directory: watchPath, file: linkName });
          expect(firedEvents[1]).toEqual({ action: nsfw.actions.CREATED, directory: watchPath, file: linkName });
          return watch.stop();
        })
        .then(done);
    });
