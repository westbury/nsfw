  ...
  describe('followSymlinks', () => {
    ...
    it('fires events for the symlink itself when followSymlinks is false', (done) => {
      ...
      nsfw(watchPath, storeEvents, { debounceMS: DEBOUNCE, followSymlinks: false })
        ...
        .then(() => new Promise(resolve => setTimeout(resolve, TIMEOUT_PER_STEP)))
        .then(() => {
          assert.equal(firedEvents.length, 2);
          assert.deepEqual(firedEvents[0], { action: nsfw.actions.DELETED, directory: watchPath, file: linkName });
          assert.deepEqual(firedEvents[1], { action: nsfw.actions.CREATED, directory: watchPath, file: linkName });
          return watch.stop();
        })
        .then(done);
    });
